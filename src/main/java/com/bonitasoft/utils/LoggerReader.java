...
public class LoggerReader {
    ...
    private Map<String, Long> processesEnd = new HashMap<>();
    private Map<String, Long> processesStart = new HashMap<>();
...
/*
    ...
    private void printProcessStats() {
        System.out.println("-----------------------------------------------------------------------------------");
        ...
        System.out.println();
        System.out.println("-----------------------------------------------------------------------------------");
        System.out.println("----------------                 CASES PER PROCESS START          -----------------");
        System.out.println("-----------------------------------------------------------------------------------");
        for(String process : processesStart.keySet()){
            System.out.println(process + ": " + processesStart.get(process));
        }
        System.out.println();
        System.out.println();
        System.out.println();
        System.out.println("-----------------------------------------------------------------------------------");
        System.out.println("----------------                CASES PER PROCESS END             -----------------");
        System.out.println("-----------------------------------------------------------------------------------");
        for(String process : processesEnd.keySet()){
            System.out.println(process + ": " + processesEnd.get(process));
        }
    ...
    private void readProcessTracking(String line) {
        if(line.contains("PROCESS TRACKING - START - MainProcess:")) {
            int pos = line.indexOf("PROCESS TRACKING - START - MainProcess:") + "PROCESS TRACKING - START - MainProcess:".length();
            StringTokenizer st = new StringTokenizer(line.substring(pos), " - ");

            String process = st.nextToken();
            Long c = processesStart.get(process);
            if (c == null) {
                c = 0L;
            }
            c++;
            processesStart.put(process, c);
        }else {
            ...
            if (line.contains("PROCESS TRACKING - END - MainProcess:")) {
                ...
                boolean exit = false;
                String process = st.nextToken();
                Long c = processesEnd.get(process);
                if (c == null) {
                    c = 0L;
                }
                c++;
                processesEnd.put(process, c);
...
*/
                pos = line.indexOf("- took: ") + "- took: ".length();
                String time = line.substring(pos).trim();
                pos = time.indexOf(" ") + 1;
                time = time.substring(pos).trim();
